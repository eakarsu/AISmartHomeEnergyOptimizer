// AI Load shifting optimizer
// Predict energy price, adjust consumption timing of high-load devices
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AISmartHomeEnergyOptimizer'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AISmartHomeEnergyOptimizer. Focus area: Load shifting optimizer. ${`Predict energy price, adjust consumption timing of high-load devices`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Load shifting optimizer.\n${`Predict energy price, adjust consumption timing of high-load devices`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'load-shift', context, parsed);
    res.json({ feature: 'load-shift', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[load-shift]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['load-shift']);
